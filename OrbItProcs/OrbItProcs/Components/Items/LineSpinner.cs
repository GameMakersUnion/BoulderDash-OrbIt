...
namespace OrbItProcs
{
    ...
    public class LineSpinner : Component
    {
        ...
        public void RandomizeSpinningLines()
        {
            ...
            float? copyOffset = Utils.random.Next(100) % 2 == 0 ? (float)Utils.random.NextDouble() * GMath.PIbyTwo: (null as float?);//as if

            int distCopyCount = Utils.random.Next(maxDistCopyCount) + 1;
            for (int i = 0; i < distCopyCount; i++)
            {
                SpinningLine spin = new SpinningLine(center, rotationSpeed, totalRotationSpeed, minDist, maxDist + minDist, speed, numberOfLines, copyCount, randmaxPermanentDraw, copyOffset, parent.body.color);
    ...
    public class SpinningLine
    {
        ...
        public SpinningLine(Vector2 center, float rotationSpeed, float totalRotationSpeed, float minDist, float maxDist, float speed, int lineCount, int copyCount, int permDraw, float? copyOffset, Color? color = null)
        {
            this.center = center;
            this.rotation = 0;
        ...
        public void UpdateLines()
        {
            ...
            else if (dist < minDist) { dist = minDist; speed *= -1; }
        }
        public void DrawLines(LineSpinner lineSpinner, Color newColor)
        {
            for (int o = 0; o < copyCount; o++)
            {
                for (int i = 0; i < lineCount; i++)
                {
                    ...
                    lineSpinner.room.camera.AddPermanentDraw(textures.whitepixel, dir + lineSpinner.parent.body.pos, newColor, new Vector2(1f, length), rotationAngle, permDraw);
